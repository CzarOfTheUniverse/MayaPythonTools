"""
~ Weight Tools ~ Christopher M. Miller ~ 2016/10/25
...
myFile = os.path.join(myDir, 'CMiller_WeightTools.ui')
...
class CMiller_WeightJumper(QtGui.QDialog):
    def __init__(self, parent=getMayaWindow()):
        ...
        self.UI.swap_pushButton.clicked.connect(self.swapUI)
        ...
        self.UI.destJoint_lineEdit.returnPressed.connect(self.loadNew)
        self.UI.sourceJoint_lineEdit.returnPressed.connect(self.loadNew)
        self.UI.sourceMesh_lineEdit.returnPressed.connect(self.loadNew)
    ...
    def loadNew(self):
        ...
        elif sender=="sourceJoint_lineEdit":
            txt = self.UI.sourceJoint_lineEdit.text()
            skinClust = list(set(cmds.listConnections(txt,type="skinCluster")))

        # Destination Joint
        elif sender == "loadDest_pushButton":
            self.UI.destJoint_lineEdit.setText(sel)
            skinClust = list(set(cmds.listConnections(sel,type="skinCluster")))
        elif sender=="destJoint_lineEdit":
            txt = self.UI.destJoint_lineEdit.text()
            skinClust = list(set(cmds.listConnections(txt,type="skinCluster")))
        ...
        elif sender=="sourceMesh_lineEdit":
            txt = self.UI.sourceMesh_lineEdit.text()
            skinClust = list([mel.eval("findRelatedSkinCluster %s"%txt)])
    ...
    def swapUI(self):
        """Swaps the Source and Destination boxes in the UI"""
        dTxt = self.UI.destJoint_lineEdit.text()
        sTxt = self.UI.sourceJoint_lineEdit.text()

        self.UI.destJoint_lineEdit.setText(sTxt)
        self.UI.sourceJoint_lineEdit.setText(dTxt)
